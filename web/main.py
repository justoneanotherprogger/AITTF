  ...
  <div class="flex flex-1 overflow-hidden">
    ...
    <main class="flex-1 flex flex-col">
      ...
      <div class="border-t border-gray-700 bg-gray-800 p-4">
        <form hx-post="/send_message" hx-target="#chat-messages" hx-swap="afterbegin"
              hx-indicator="#spinner" hx-on--after-request="this.reset()"
              class="flex gap-2 mb-2">
          <input type="text" name="message" placeholder="Сказать что-то..." required
        ...
        <form hx-post="/declare_action" hx-target="#chat-messages" hx-swap="afterbegin"
              hx-indicator="#spinner" hx-on--after-request="this.reset()"
              hx-on::before-request="document.getElementById('action-btn').disabled=true; document.getElementById('action-btn').textContent='Думает...'"
              hx-on::after-request="document.getElementById('action-btn').disabled=false; document.getElementById('action-btn').textContent='Заявить действие'"
              class="flex gap-2">
          <input type="text" name="action" placeholder="Описать действие..." required
                 class="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500">
...
async def send_message(request: Request, message: str = Form(...)):
    try:
        ...
        extend_timer()
        turn = await process_player_action(player_id, message, is_action=False)
    except Exception as e:
    ...
    conn = get_connection()
    rows = conn.execute(
        "SELECT sender, message_text, is_action FROM chat_history ORDER BY id DESC LIMIT 2"
    ).fetchall()
...
async def declare_action(request: Request, action: str = Form(...)):
    try:
        player_id = int(request.cookies.get("player_id", 0))
        extend_timer()
        turn = await process_player_action(player_id, action)
    except Exception as e:
        print(f"ERROR IN ENDPOINT /declare_action: {e}")
        return '<div class="text-red-400 text-sm">Ошибка обработки действия</div>'
    conn = get_connection()
    rows = conn.execute(
        "SELECT sender, message_text, is_action, action_type FROM chat_history ORDER BY id DESC LIMIT 4"
    ).fetchall()
    player_rows = conn.execute("SELECT * FROM players").fetchall()
    conn.close()
